...
def parse_ip_packet(packet_hex):
    ...
    # Wersja protokołu (pierwsza cyfra pierwszego bajtu)
    version = (packet_bytes[0] >> 4) & 0xF

    # Adresy IP (4 bajty każdy, zaczynają się od 12 bajtu nagłówka IP)
    src_ip = "".join(map(str, packet_bytes[12:16]))
    dst_ip = "".join(map(str, packet_bytes[16:20]))

    # Typ protokołu wyższej warstwy (9 bajt nagłówka IP)
    protocol_type = packet_bytes[9]

    # TCP (protokół 6) - analiza nagłówka TCP
    if protocol_type == 6:
        src_port, dst_port = struct.unpack('!HH', packet_bytes[20:24])
        header_length = (packet_bytes[32] >> 4) * 4  # długość nagłówka TCP w bajtach
        data_offset = 20 + header_length  # Nagłówek IP (20B) + TCP
...
def main():
    ...
    # Debugowanie wartości przed wysłaniem
    ...
    # Jeśli pierwsza odpowiedź jest poprawna, wysyłamy drugą wiadomość
